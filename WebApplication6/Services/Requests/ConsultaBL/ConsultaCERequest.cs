...
{
    public class ConsultaCERequest
    {
        private readonly CookieRepository _cookieRepository;

        public ConsultaCERequest(CookieRepository cookieRepository)
        {
            _cookieRepository = cookieRepository;
        }
        public async Task<object> ConsultaCE(string filCod, string usnCod, string refExt)
        {
            ...
            using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<RootObjectCE>(responseContent);
