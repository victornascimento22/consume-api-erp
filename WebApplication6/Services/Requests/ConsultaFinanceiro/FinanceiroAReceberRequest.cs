...
{
    public class FinanceiroAReceberRequest
    {
        public class ConsultaFinanceiroAReceberRequest
        {
            private readonly CookieRepository _cookieRepository;

            public ConsultaFinanceiroAReceberRequest(CookieRepository cookieRepository)
            {
                _cookieRepository = cookieRepository;
            }

            public async Task<object> ConsultaFinanceiroAReceber(string filCod, string usnCod, string refExt)
            {
                ...
                using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
                {
                    ...
                    {
                        var obj = JsonConvert.DeserializeObject<CnxListResponseFinanceiroAReceberDTO>(responseContent);
                        return obj;
                    }
