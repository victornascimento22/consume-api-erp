...
using static WebApplication6.DTO.Produtos.ConsultaProdutos.ConsultaProdutoDTO;
using static WebApplication6.DTO.Produtos.ConsultaProdutos;

namespace WebApplication6.Services.ConsultaProdutos
{
    public class ConsultaProdutosRequest
    {
        private readonly CookieRepository _cookieRepository;

        public ConsultaProdutosRequest(CookieRepository cookieRepository)
        {
            _cookieRepository = cookieRepository;
        }

        public async Task<object> ConsultaProdutos(string filCod, string usnCod, string prdCod)
        {
            var url = "https://capital-homologacao.conexos.cloud/api-etl/capital/consultaProduto";
            ...
            using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
            {
                ...
                {
                    var obj = JsonConvert.DeserializeObject<RootObjectProdutos>(responseContent);
                    return obj;
                }
