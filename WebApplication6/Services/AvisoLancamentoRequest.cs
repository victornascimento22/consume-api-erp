...
{
        public class ConsultaAvisoLancamentoRequest
        {
            private readonly CookieRepository _cookieRepository;

            public ConsultaAvisoLancamentoRequest(CookieRepository cookieRepository)
            {
                _cookieRepository = cookieRepository;
            }

            public async Task<object> ConsultaAvisoLancamento(string filCod, string usnCod, string refExt)
            {
                var url = "https://capital-homologacao.conexos.cloud/api-etl/capital/consultaAvisoLancamento";
                ...
                using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
                {
                    ...
                    {
                        var obj = JsonConvert.DeserializeObject<CnxListResponseAvisoLancamentosDTO>(responseContent);
                        return obj;
                    }
