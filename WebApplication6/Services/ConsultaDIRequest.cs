...
public class ConsultaDIRequest
{
    private readonly CookieRepository _cookieRepository;

    public ConsultaDIRequest(CookieRepository cookieRepository)
    {
        _cookieRepository = cookieRepository;
    }

    public async Task<object> ConsultaDI(string filCod, string usnCod, string refExt)
    {
        var url = "https://capital-homologacao.conexos.cloud/api-etl/capital/consultaDi";
        ...
        using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
        {
            ...
            {
                var obj = JsonConvert.DeserializeObject<CnxListResponseDIDTO>(responseContent);
                return obj;
            }
