...
{
    public class ConsultaItensInvoiceRequest
    {
        private readonly CookieRepository _cookieRepository;

        public ConsultaItensInvoiceRequest(CookieRepository cookieRepository)
        {
            _cookieRepository = cookieRepository;
        }
        public async Task<object> ConsultaItensInvoice(string filCod, string usnCod, string invEspNumero)
        {
            var url = "https://capital-homologacao.conexos.cloud/api-etl/capital/consultaItensInvoice";
            ...
            using (var httpClient = new HttpClient(new HttpClientHandler { CookieContainer = cookieContainer }))
            {
                ...
                var queryParams = $"?filCod={filCod}&invEspNumero={invEspNumero}";
                ...
                {
                    var obj = JsonConvert.DeserializeObject<CnxListResponseItensInvoiceDTO>(responseContent);
